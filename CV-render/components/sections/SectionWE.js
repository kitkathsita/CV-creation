	...
	return (
		<>
			<div id="section">{sectionName}</div>
			{
				role.map(job => {
					i++
					return <WorkExperience key={i+'we'} position={job.position} company={job.company} period={job.period} jobDescription={job.jobDescription} tasks={job.task} place={job.place}></WorkExperience>
				})
			}
		</>
	)
